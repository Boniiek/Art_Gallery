	...
	if ($_POST["u_status"] == 2) {
		...
		if ($num_r <> 0) {
			...
			if(password_verify($user_password, $user_password_r) and $user_status == 0) {
		        ...
		        $_COOKIE["msg"] = 1;
			    echo "<script>window.location = 'drawings.php'</script>";
			}
		}
		else {
			setcookie('msg', 1, time()+1);
...
<div id="navbar">
  ...
  <a class="active" href="drawings.php"><span lang="en">Drawings</span><span lang="ru">Рисунки</span></a>
  ...
  if ($_COOKIE["login"] == 0) {
	  ...
	  $u_na = $_COOKIE["uname"];
	  echo "<table><td><a>$u_na</a></td><td><form action='drawings.php' method='post'><input type='hidden' id='u_status' name='u_status' value='0'><button type='submit'><span lang='en'>Log out</span><span lang='ru'>Выйти</span></button></form></td>
